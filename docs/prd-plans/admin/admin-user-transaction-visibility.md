...
- Given two transactions of different types are displayed, when the operator views them, then each row uses its designated color (Deposit=Green, Entry=Blue, Slip Entry=Light Blue, Payout=Gold, Slip Payout=Yellow, Contest/Entry Canceled=Grey, Withdrawal=Red, Bonus=Teal).
